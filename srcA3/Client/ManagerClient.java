...
public class ManagerClient  {
	...
	public static void singleThread() throws Exception{
		...
		CenterServer demsServer = connectToServer(serverID);
	...
	public void editRecord(CenterServer demsServer) {
	...
	public void getRecordCount (CenterServer demsServer) {

		System.out.println("Record Count");
		String result = demsServer.getRecordCounts(managerID);//stuck here
		System.out.println(result);//null 
		LOGGER.info(String.format(Config.GET_RECORDS_COUNT, managerID, result));
	}
	public void transferRecord (CenterServer demsServer) {
	...
	private static CenterServer connectToServer(Config.Server_ID serverID)  {
		try {
			switch(serverID) {
			case CA:{
				String server="ca";
				URL url = new URL("http://localhost:8080/CenterServerImpl/" + server
						+ "?wsdl");
				QName qName = new QName("http://Servers/",
						"CenterServerImplService");
				Service service = Service.create(url, qName);
				demsServer = service.getPort(CenterServer.class);
				return demsServer;
			...
			case US:{
				...
				url = new URL("http://localhost:8080/CenterServerImpl/" + server
						+ "?wsdl");
				QName qName = new QName("http://Servers/",
						"CenterServerImplService");
				Service service = Service.create(url, qName);
			...
			System.out.println("Error on ManagerClient");
			e.printStackTrace();
		}
